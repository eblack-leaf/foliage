...
fn coast(grove: &mut Grove, elements: &Elements) {
...
fn sought(grove: &mut Grove, elements: &Elements) {
    for (leaf, to) in core::mem::take(&mut grove.sought) {
        if let Some(landed) = destination(grove, elements, leaf, to) {
...
fn animated(grove: &mut Grove, elements: &Elements) {
    for (leaf, from, to, at) in grove.aspen.scrolling() {
        let Some(target) = destination(grove, elements, leaf, to) else {
            continue;
        };
