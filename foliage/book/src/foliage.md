...
- [The Primitives](./primitives.md) — the visual/positioning/animation vocabulary
  (`Panel`/`Text`/`Icon`/`Image`, `Location`/`Grid`/`Elevation`, `Animation`/`Sequence`) that
  authoring composes, before authoring itself is introduced.
