...
activity, and `androidx.games:games-activity` — the AAR behind winit's `android-game-activity`
backend — can only be linked by Gradle. [`foliage-android`](foliage-android) generates that project
and then drives it, so the parts that are unavoidable are at least not manual:
...
foliage-android run      # every configured ABI, assembled, installed and launched
```

`init` writes the Gradle project, the `cdylib` entry crate holding `android_main`, and
`foliage-android.toml` recording every choice — so no later command takes arguments, and rerunning
`init` regenerates against an edited file. `build` resolves the SDK from the environment or from
`.android-sdk/` beside the repo and sets it on the processes it starts, rather than expecting a
shell that was prepared first.

An app's own code is unchanged. Only construction differs per platform:
...
| [`foliage-android/`](foliage-android) | The Android tool: scaffolds the Gradle project an app needs, then builds and runs it. |
...
| [`application-android/`](application-android) | `application`'s Android entry point, generated by `foliage-android init`. One `android_main` and nothing else. The Gradle project it is packaged into is generated beside the app, at `application/android/`, and is not committed. |
| [`book/`](book) | The book. |
...
| `api` | Builds the API reference for `foliage`, `foliage-icons` and `foliage-android` into `docs/api`. | — |
