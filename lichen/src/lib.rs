...
//!
//! Three arrangements of it. A [`Mosaic`] fills a [`Silhouette`] -- any closed outline -- can
//! stand a square region on any of the outline's vertices, and can be cropped down to any part of
//! the outline and put back. A [`Trail`] runs a line of beads between two points. A [`Cluster`]
//! puts a puff of motes in a box the caller already grew.
//!
...
//! is the look, and it is not a knob. How large a region is can be, because what stands on one is
//! the caller's and is what a region is sized for.
