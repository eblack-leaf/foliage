...
/// Chapter 3: how a parent divides itself into columns/rows so children can address
/// cells with `.col()`/`.row()` -- and why any child with a `Location` needs its parent
/// to carry one. Placeholder shape only; the real infographic for this concept lands
/// later.
