...
use crate::boundary::canopy::Canopy;
...
/// exclusively through the [`Canopy`] it is lent.
pub trait Root: Sized + 'static {
    /// Grows the tree and returns the app.
    ///
    /// Called once, at the first frame, with a `Canopy` no different from any other frame's:
...
pub(crate) trait Rooted {
    fn frame(&mut self, canopy: &mut Canopy<'_, '_>, blooms: Vec<Bloom>);
}

/// A registered root before and after it has taken. The engine cannot build one at
/// registration time -- growing a tree needs a live `Canopy`, which only exists inside a
...
impl<R: Root> Rooted for Planted<R> {
    fn frame(&mut self, canopy: &mut Canopy<'_, '_>, blooms: Vec<Bloom>) {
        let root = match &mut self.0 {
            Some(root) => root,
            slot => slot.insert(R::take_root(canopy)),
        };
        root.frame(canopy, blooms);
    }
}
