...
mod tests {
    ...
    use crate::{EcsExtension, Foliage, Logical, Section, Sprout};

    #[test]
    fn debug_opened_surface_geometry() {
        let mut foliage = Foliage::new();
        foliage
            .world
            .insert_resource(ViewportHandle::new(Area::logical((800.0, 600.0))));

        let popover = foliage.world.leaf(
            Popover::new()
                .trigger(|tree, slot| {
                    tree.branch(slot, crate::Leaf::sprout().at(crate::Location::new()).elevate(crate::Elevation::up(1)))
                })
                .content(|tree, slot| {
                    tree.branch(slot, crate::Leaf::sprout().at(crate::Location::new()).elevate(crate::Elevation::up(1)))
                })
                ...
                .at(Location::new().xs(
                    ...
                    20.px().as_top().with(30.px().as_height()),
                ))
                .elevate(crate::Elevation::up(1)),
        );
        foliage.world.flush();
        ...
        let mut q = foliage
            .world
            .query::<(Entity, &Panel, &Section<Logical>, &crate::ash::clip::ResolvedClip, &crate::opacity::BlendedOpacity, &crate::Visibility)>();
        let results: Vec<_> = q
            .iter(&foliage.world)
            .map(|(e, _, s, c, o, v)| (e, *s, c.0, o.value, v.visible()))
            .collect();
        eprintln!("panels found: {}", results.len());
        for (e, s, c, o, v) in &results {
            eprintln!(
                "entity {:?}: section=(left={} top={} width={} height={}) clip=(left={} top={} width={} height={}) opacity={} visible={}",
                e,
                s.left(),
                s.top(),
                s.width(),
        ...
        assert!(!results.is_empty(), "should have found at least the surface's own Panel");
    }
}
