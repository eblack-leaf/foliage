...
use crate::grid::location::ResolvedLocation;
use crate::text::monospaced::MonospacedFont;
use crate::visibility::AutoVisibility;
use crate::{Animate, AspectRatio, Attachment, Component, CoordinateUnit, Coordinates, Foliage, FontSize, Grid, Layout, Line, Logical, Points, ResolvedVisibility, Section, Stem, Tree, Update, View, Visibility, Write};
...
impl crate::Stack {
    ...
    fn on_insert(mut world: DeferredWorld, this: Entity, _c: ComponentId) {
        let stack = world.get::<crate::Stack>(this).unwrap();
        if let Some(id) = stack.id {
            if let Some(mut deps) = world.get_mut::<crate::StackDeps>(id) {
                deps.ids.insert(this);
            } else {
                let mut stack_deps = crate::StackDeps::default();
    ...
    fn on_replace(mut world: DeferredWorld, id: Entity, _c: ComponentId) {
        let stack = world.get::<crate::Stack>(id).unwrap();
        if let Some(id) = stack.id {
            if let Some(mut deps) = world.get_mut::<crate::StackDeps>(id) {
                deps.ids.remove(&id);
            }
